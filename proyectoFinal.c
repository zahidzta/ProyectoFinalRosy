...
void modificarRegisro(){
	while(i != 0){
			...
			if(i == 1){
				...
				scanf("%d", &d);
				//cliclo for que  va a remplazar la posison de la clave que se desea
				for( x=0; x<10; x++){
					//sustituir la clave original por la nueva
					if(clave[x] == c){
					clave[x] = d;
					}
				}									
			}
			//Para modificar el nombre, le probe de muchas maneras pero por alguna razon nome lee lo que ingreso
			//y nunca me deja escribir el nombre nuevoq ue quiero ingresar
			//los otros 2 ya funcionan, no se si quieras que les agregue algo mas
			// para el 2 segui lit la misma logica pero por alguna rzon no funciona, me da error en los "="
			/*if(i == 2){
				char c, x, d; 
				//Modificar nombre 
				system("cls");
				...
				for( x=0; x<10; x++){
					if( nombresGenerados[x] == c){
						nombresGenerados[x] == d;
					}
				}
			}*/
			if(i == 3){
				...
				scanf("%d", &d);
				//cliclo for que  va a remplazar la posison de el slario que se desea
				for( x=0; x<10; x++){
					//sustituir el salario original por el nuevo
					if(salario[x] == c){
						salario[x] = d;
					}
				}
